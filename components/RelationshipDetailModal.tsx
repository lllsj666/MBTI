  ...
  return (
    ...
    >
      <div
        ...
        className="relative flex max-h-[90vh] w-full animate-[slideUp_0.25s_ease-out] flex-col overflow-hidden rounded-t-[28px] bg-white/95 shadow-2xl shadow-slate-900/20 backdrop-blur-xl md:max-h-[88vh] md:w-[min(92vw,680px)] md:rounded-[28px]"
      >
        {/* Close */}
        <button
          onClick={onClose}
        ...
        {/* Header — identity only, no description */}
        ...
        {/* Body — vertical scroll, 5 cards */}
        <div className="flex-1 overflow-y-auto px-5 py-5 pb-[calc(24px+env(safe-area-inset-bottom,0px))] md:px-8 md:py-6">
          <div className="space-y-4">
            <CardsSection label="完整描述">
              {textCards(item.description)}
            </CardsSection>

            <CardsSection label="对方特点">
              {textCards(item.traits)}
            </CardsSection>

            <CardsSection label={variant === "match" ? "你们的默契点" : "你们的磨合点"}>
              {textCards(variant === "match" ? item.chemistry : item.frictionPoint)}
            </CardsSection>

            <CardsSection label="相处建议">
              {textCards(item.howToGetAlong)}
            </CardsSection>

            <CardsSection label="潜在摩擦">
              {textCards(item.possibleFriction)}
            </CardsSection>
          </div>
        </div>
      </div>
