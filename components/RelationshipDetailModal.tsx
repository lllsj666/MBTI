...
export type RelationshipDetail = {
  ...
  traits: string; chemistry: string; frictionPoint: string;
...
export function RelationshipDetailModal({ open, onClose, item, variant }: Props) {
  ...
  const cards: DeckCard[] = [
    { key: "traits", title: "对方特点", content: item.traits },
    { key: "chem", title: variant === "match" ? "你们的默契点" : "你们的磨合点", content: variant === "match" ? item.chemistry : item.frictionPoint },
    { key: "how", title: "相处建议", content: item.howToGetAlong },
    { key: "friction", title: "潜在摩擦", content: item.possibleFriction },
  ];
  ...
  return (
    <div className="fixed inset-0 z-50 flex animate-[fadeIn_0.2s_ease-out] items-end justify-center bg-slate-900/40 backdrop-blur-sm md:items-center md:p-4" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="relative flex max-h-[88vh] w-full animate-[slideUp_0.25s_ease-out] flex-col overflow-hidden rounded-t-3xl bg-white/95 shadow-2xl backdrop-blur-xl md:max-w-xl md:rounded-3xl">
        {/* Close */}
        ...
        {/* Swipeable deck */}
        <div className="flex-1 overflow-hidden px-6 py-4 md:px-8">
