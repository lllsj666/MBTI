...
const dimCards = [
  { grad: "from-violet-100/60 to-purple-50/40", accent: "bg-violet-400/30", title: "社交能量", desc: "你从人群还是独处中恢复状态" },
  { grad: "from-emerald-100/60 to-teal-50/40", accent: "bg-emerald-400/30", title: "信息偏好", desc: "你更关注现实细节还是潜在可能" },
  { grad: "from-amber-100/60 to-yellow-50/40", accent: "bg-amber-400/30", title: "决策方式", desc: "你更常先看逻辑还是先照顾感受" },
  { grad: "from-sky-100/60 to-blue-50/40", accent: "bg-sky-400/30", title: "生活节奏", desc: "你偏好提前规划还是保留弹性" },
];
...
export default function Home() {
  ...
  return (
    <main className="relative flex-1 overflow-hidden bg-[var(--bg)] text-[var(--text)]">
      {/* Ambient glows */}
      <div className="pointer-events-none fixed -top-40 -left-20 h-[600px] w-[600px] rounded-full bg-violet-300/15 blur-[140px]" />
      ...
      <nav className="sticky top-0 z-50 border-b border-[var(--border)] bg-[var(--surface)] backdrop-blur-xl">
        <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-5">
          ...
          <Link href="/test" className="rounded-full bg-[var(--text)] px-5 py-2 text-sm font-medium text-[var(--bg)] shadow-sm transition hover:opacity-90 active:scale-95">
            开始测试
          </Link>
        </div>
      </nav>

      {/* Hero + Preview — PC: side by side */}
      <section className="relative z-10 mx-auto max-w-6xl px-5 pb-16 pt-12 sm:pt-20 lg:flex lg:items-center lg:gap-16 lg:pb-24 lg:pt-24">
        {/* Left: text */}
        <motion.div className="flex-1 lg:pr-8" initial="offscreen" animate="onscreen" variants={stagger}>
          ...
          <motion.h1 variants={fadeUp} className="mb-5 text-4xl font-bold leading-tight tracking-tight sm:text-5xl lg:text-6xl">
            你是怎样和世界
            <span className="bg-gradient-to-r from-violet-500 via-fuchsia-500 to-amber-400 bg-clip-text text-transparent">相处</span>
            的？
          </motion.h1>
          <motion.p variants={fadeUp} className="mb-6 max-w-md text-base leading-relaxed text-[var(--muted)] sm:text-lg">
            回答 36 个轻松场景题，从社交能量、信息偏好、决策方式和生活节奏里，翻开你更真实的性格底色。
          </motion.p>
          <motion.div variants={fadeUp} className="mb-8 flex flex-wrap gap-2">
            {dimChips.map(({ label }) => (
              <span key={label} className="rounded-full border border-[var(--border)] bg-[var(--surface)] px-3 py-1 text-xs text-[var(--muted)] backdrop-blur-sm">
                {label}
              </span>
            ))}
          </motion.div>
          <motion.div variants={fadeUp} className="flex flex-col items-start gap-3 sm:flex-row">
            <Link href="/test" className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-violet-600 to-fuchsia-600 px-8 py-3.5 text-sm font-medium text-white shadow-lg shadow-violet-200/30 transition hover:-translate-y-0.5 hover:shadow-xl active:scale-95">
              开始测试
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M12 5l7 7-7 7" /></svg>
            </Link>
          </motion.div>
          <motion.p variants={fadeUp} className="mt-5 text-xs text-[var(--muted)]">约 5-7 分钟 · 免费 · 无需注册 · 仅供自我探索参考</motion.p>
        </motion.div>

        {/* Right: preview card (desktop only shows on larger screens) */}
        <motion.div className="mt-12 flex-1 lg:mt-0" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0, transition: { duration: 0.6, delay: 0.2 } }}>
          <div className="relative overflow-hidden rounded-3xl border border-[var(--border)] bg-[var(--surface)] shadow-xl backdrop-blur-xl">
            {/* Inner glow */}
            <div className="pointer-events-none absolute -top-16 left-1/2 h-[200px] w-[300px] -translate-x-1/2 rounded-full bg-violet-300/15 blur-[80px]" />
            <div className="relative p-6 sm:p-8">
              ...
              <div className="mb-5 flex flex-wrap gap-1.5">
                {randomExample.keywords.map((k: string) => (
                  <span key={k} className="rounded-full border border-[var(--border)] bg-[var(--surface)] px-2.5 py-0.5 text-xs text-[var(--muted)]">{k}</span>
                ))}
              </div>
      ...
      <motion.section className="relative z-10 px-5 py-16 sm:py-20" initial="offscreen" whileInView="onscreen" viewport={{ once: true }} variants={stagger}>
        ...
        <div className="mx-auto grid max-w-5xl gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {dimCards.map(({ grad, accent, title, desc }) => (
      ...
      <motion.section className="relative z-10 px-5 pb-16 sm:pt-4" initial="offscreen" whileInView="onscreen" viewport={{ once: true }}>
        <motion.div variants={fadeUp} className="mx-auto max-w-xl rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-8 text-center shadow-sm backdrop-blur-xl sm:p-10">
          ...
          <h2 className="mb-4 text-2xl font-bold tracking-tight sm:text-3xl">准备好看见你的<span className="bg-gradient-to-r from-violet-500 to-fuchsia-500 bg-clip-text text-transparent">性格底色</span>了吗？</h2>
      ...
      <footer className="relative z-10 border-t border-[var(--border)] bg-[var(--surface)]/40 px-5 py-8 backdrop-blur-sm">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 text-sm text-[var(--muted)] sm:flex-row">
          <span className="flex items-center gap-1.5 font-semibold"><span className="h-2 w-2 rounded-full bg-gradient-to-br from-violet-500 to-fuchsia-500" />MBTI 性格测试-覆</span>
