...
const CAMP_COLORS: Record<string, { textGrad: string; btnGrad: string; shadow: string }> = {
  analyst:  { textGrad:"from-violet-400 to-fuchsia-400", btnGrad:"from-violet-500 to-fuchsia-600", shadow:"shadow-violet-200/40" },
  diplomat: { textGrad:"from-emerald-400 to-teal-400",   btnGrad:"from-emerald-500 to-teal-600",   shadow:"shadow-emerald-200/40" },
  sentinel: { textGrad:"from-sky-400 to-blue-500",       btnGrad:"from-sky-500 to-blue-600",       shadow:"shadow-sky-200/40" },
  explorer: { textGrad:"from-amber-300 to-yellow-400",   btnGrad:"from-amber-500 to-orange-500",   shadow:"shadow-amber-200/40" },
};
...
export default function Home() {
  ...
  const randomExample = useMemo(() => {
    ...
    const camps: Record<string, keyof typeof CAMP_COLORS> = { "分析家":"analyst","外交家":"diplomat","守护者":"sentinel","探险家":"explorer" };
    const accent = CAMP_COLORS[camps[camp.label] || "analyst"];
    ...
    return { type: t, name: data.name, keywords: data.keywords.slice(0,4), camp, accent, chips };
  }, []);
      ...
      <nav className="sticky top-0 z-50 border-b border-[var(--border)] bg-[var(--surface)] backdrop-blur-xl">
        <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-5">
          <Link href="/" className="flex items-center gap-2 text-lg font-bold tracking-tight text-[var(--text)]">
            <span className={`h-2.5 w-2.5 rounded-full bg-gradient-to-r ${randomExample.accent.textGrad}`} />
            覆
          </Link>
          <Link href="/test" className={`rounded-full bg-gradient-to-r ${randomExample.accent.btnGrad} px-5 py-2 text-sm font-medium text-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md active:scale-95`}>
            开始测试
          </Link>
        </div>
      </nav>

      {/* Hero + Preview — PC: card LEFT, text RIGHT */}
      <section className="relative z-10 mx-auto max-w-6xl px-5 pb-16 pt-12 sm:pt-20 lg:flex lg:flex-row-reverse lg:items-center lg:gap-16 lg:pb-24 lg:pt-24">
        ...
        <motion.div className="flex-1 text-center lg:text-left" initial="offscreen" animate="onscreen" variants={stagger}>
          ...
          <motion.h1 variants={fadeUp} className="mb-5 text-4xl font-bold leading-tight tracking-tight sm:text-5xl lg:text-6xl">
            你是怎样和世界
            <span className={`bg-gradient-to-r ${randomExample.accent.textGrad} bg-clip-text text-transparent`}>相处</span>
            的？
          </motion.h1>
          ...
          <motion.div variants={fadeUp} className="flex justify-center lg:justify-start">
            <Link href="/test" className={`inline-flex items-center gap-2 rounded-xl bg-gradient-to-r ${randomExample.accent.btnGrad} px-8 py-3.5 text-sm font-medium text-white shadow-lg ${randomExample.accent.shadow} transition hover:-translate-y-0.5 hover:shadow-xl active:scale-95`}>
              开始测试
      ...
      <motion.section className="relative z-10 px-5 pb-16 sm:pt-4" initial="offscreen" whileInView="onscreen" viewport={{ once: true }}>
        <motion.div variants={fadeUp} className="mx-auto max-w-xl rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-8 text-center shadow-sm backdrop-blur-xl sm:p-10">
          ...
          <h2 className="mb-4 text-2xl font-bold tracking-tight sm:text-3xl">
            准备好看见你的<span className={`bg-gradient-to-r ${randomExample.accent.textGrad} bg-clip-text text-transparent`}>性格底色</span>了吗？
          </h2>
